...
class ModelRunner:
    ...
    @property
    def post_capture_kv_active(self) -> bool:
        return (
    ...
    def post_capture_resize_kv_pool(self) -> None:
        ...
        config = self.kv_cache_configurator._config_from_budget(
    ...
    def _init_unified_mamba_pools(self, max_num_reqs: int):
    ...
    def _init_unified_swa_pools(self, max_num_reqs: int):
...
def capture_cuda_graphs(
    *, model_runner: ModelRunner, capture_decode_cuda_graph: bool = True
    ...
    mr.graph_shared_output = GraphSharedOutput.create_for_model_runner(mr)
    ...
    if capture_decode_cuda_graph:
        if mr.device in ("cuda", "musa", "cpu", "npu", "xpu"):
...
def capture_decode_graph(*, model_runner: ModelRunner) -> DecodeGraphCapture:
    ...
    if mr.device != "cpu" and check_cuda_graph_backend(Phase.DECODE, Backend.DISABLED):
        return no_capture
    if mr.device == "cpu" and not get_flags().capture.enable_torch_compile:
        return no_capture
