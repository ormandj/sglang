...
class ModelRunner:
    ...
    def init_kv_cache_configurator(self):
        self.kv_cache_configurator = KVCacheConfigurator(
            ...
            spec_algorithm=self.spec_algorithm,
            is_draft_worker=self.is_draft_worker,
            post_capture_kv_active=is_post_capture_kv_active(self),
    ...
    def alloc_memory_pool(self, memory_pool_config: Optional[MemoryPoolConfig] = None):
        ...
        if (
            ...
            if mambaish_config(self.model_config) is not None:
                build_unified_mamba_pools(self, self.max_running_requests)
...
def post_capture_resize_kv_pool(model_runner: ModelRunner) -> None:
    ...
    config = model_runner.kv_cache_configurator._config_from_budget(
    ...
    model_runner.max_total_num_tokens = config.max_total_num_tokens
    if model_runner.is_hybrid_swa:
        model_runner.full_max_total_num_tokens = config.full_max_total_num_tokens
        model_runner.swa_max_total_num_tokens = config.swa_max_total_num_tokens
    if model_runner.memory_pool_config is not None:
        model_runner.memory_pool_config.max_total_num_tokens = (
            config.max_total_num_tokens
        )
        model_runner.memory_pool_config.full_max_total_num_tokens = (
            config.full_max_total_num_tokens
        )
        model_runner.memory_pool_config.swa_max_total_num_tokens = (
    ...
    if model_runner.max_running_requests is not None:
        ...
        capped_reqs = min(
            ...
            model_runner.kv_cache_configurator._resolve_max_num_reqs(
                config.max_total_num_tokens
            ),
        )
        if capped_reqs < model_runner.max_running_requests:
            ...
            model_runner.max_running_requests = capped_reqs
            if model_runner.memory_pool_config is not None:
                model_runner.memory_pool_config.max_running_requests = capped_reqs
    logger.info(
...
def build_unified_mamba_pools(model_runner: ModelRunner, max_num_reqs: int) -> None:
    ...
    config = mambaish_config(model_runner.model_config)
    assert config is not None
    assert (
        not model_runner.use_mla_backend
    ...
    ), f"page_size must be >= 1, got {model_runner.page_size}"
    ...
    mamba_layer_ids = [
        ...
        if model_runner.layer_info.start_layer <= i < model_runner.layer_info.end_layer
    ]
    ...
    bundle = init_unified_mamba_pools(
        ...
        model_context_len=model_runner.model_config.context_len,
        extra_max_context_len=extra_max_context_len,
        max_total_num_tokens=model_runner.max_total_num_tokens,
        max_mamba_cache_size=model_runner.server_args.max_mamba_cache_size,
        max_num_reqs=max_num_reqs,
        enable_memory_saver=model_runner.server_args.enable_memory_saver,
        enable_mamba_extra_buffer=model_runner.server_args.enable_mamba_extra_buffer(),
        speculative_num_draft_tokens=model_runner.server_args.speculative_num_draft_tokens,
        disable_overlap_schedule=model_runner.server_args.disable_overlap_schedule,
        need_sort=model_runner.server_args.disaggregation_mode in ("decode", "prefill"),
        mamba_full_memory_ratio=model_runner.server_args.mamba_full_memory_ratio,
    ...
    model_runner.req_to_token_pool = bundle.req_to_token_pool
    model_runner.token_to_kv_pool = bundle.token_to_kv_pool
    model_runner.token_to_kv_pool_allocator = bundle.token_to_kv_pool_allocator
    # Keep a reference so the shared byte buffer is not GC'd.
    model_runner._unified_memory_pool = bundle.unified_memory_pool
...
def build_unified_swa_pools(model_runner: ModelRunner, max_num_reqs: int) -> None:
    ...
    ), "build_unified_swa_pools called on a non-SWA model"
    ...
    model_runner.req_to_token_pool = ReqToTokenPool(
        size=max_num_reqs,
        max_context_len=model_runner.model_config.context_len + extra_max_context_len,
        device=model_runner.device,
        enable_memory_saver=model_runner.server_args.enable_memory_saver,
    )

    head_num = model_runner.model_config.get_num_kv_heads(get_parallel().attn_tp_size)
    head_dim = model_runner.model_config.head_dim
    if model_runner.is_hybrid_swa_compress:
        # Asymmetric head dims between full and SWA (NPU compress path):
        # pull SWA-specific dims from the hf text config.
        v_head_dim = model_runner.model_config.hf_text_config.v_head_dim
        swa_head_num = max(
            1,
            model_runner.model_config.hf_text_config.swa_num_key_value_heads
            // get_parallel().attn_tp_size,
        )
        swa_head_dim = model_runner.model_config.hf_text_config.swa_head_dim
        swa_v_head_dim = model_runner.model_config.hf_text_config.swa_v_head_dim
    else:
        v_head_dim = head_dim
    ...
    swa_attention_layer_ids = [
        i
        for i in model_runner.model_config.swa_attention_layer_ids
        if model_runner.layer_info.start_layer <= i < model_runner.layer_info.end_layer
    ]
    full_attention_layer_ids = [
        i
        for i in model_runner.model_config.full_attention_layer_ids
        if model_runner.layer_info.start_layer <= i < model_runner.layer_info.end_layer
    ]
    ...
    bundle = init_unified_swa_pools(
        ...
        full_attention_layer_ids=full_attention_layer_ids,
        full_max_total_num_tokens=model_runner.full_max_total_num_tokens,
        swa_max_total_num_tokens=model_runner.swa_max_total_num_tokens,
        enable_memory_saver=model_runner.server_args.enable_memory_saver,
        need_sort=model_runner.server_args.disaggregation_mode in ("decode", "prefill"),
